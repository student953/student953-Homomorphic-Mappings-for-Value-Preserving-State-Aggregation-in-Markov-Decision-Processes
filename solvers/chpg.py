...
class Solver:
    ...
    def run(self):
        ...
        while True:
            ...
            omega = self.env.action_dim * q_value / q_value.sum(-1, keepdims=True)
            # omega = self.env.state_dim * softmax(self.value.reshape(-1, 1), axis=0)
            self.policy_theta = (self.policy_theta + self.lr * omega * q_value)
            self.policy = softmax(self.policy_theta, axis=1)
            ...
            if (i_episode + 1) % 1 == 0:
                ...
                print_dict.update({'{}'.format(i_episode): [np.mean(self.value), end_time - start_time]})
                value_list = []
            end_time_ = time.time() - start_time
            i_episode += 1
