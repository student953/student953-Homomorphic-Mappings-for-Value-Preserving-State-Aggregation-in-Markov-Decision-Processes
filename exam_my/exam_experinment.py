...
# @File     :exam_experinment
# @Date     :2024/12/29 18:37
# @Author   : ZS
-------------------------------------------------
"""
import os
...
class Experience:
    def __init__(
            ...
            n_exp: int = 1,  # Number of experience with same parameters to measure solving time
    ...
    def run(self, model: GenericModel, add_cof = 6000):
        """
        ...
        for solver in self.solver_list:
            ...
            self._model_specific_solver_specific_experience(model, solver, add_cof)
    ...
    def _get_model_list(self):
        ...
        # self.model_list = build_model_list('exam_model')
    ...
    def _model_specific_solver_specific_experience(
            self, model: GenericModel, solver: GenericSolver, add_cof=6000
    ) -> Tuple:
        """
        ...
        # if not os.path.exists('./log/{0}_{1}.npy'.format(model.name, solver.name)):
        if True:
            all_print_save = {}
            for i in range(self.n_exp):
                print("======="*4)
                print("Slover:{0} | training time: {1}".format(solver.name, i))
                print("=======" * 4)
                solver.__init__(model,
                                self.discount,
                                max_iter_evaluation=int(1e3),
                                max_iter_policy_update=int(1e2),
                                lr=0.1,
                                max_time=int(add_cof))
                result_i = solver.run()
                all_print_save.update({'{}_result'.format(i): result_i})

            np.save('./log/{0}_{1}.npy'.format(model.name, solver.name), all_print_save)
        test1 = 1
